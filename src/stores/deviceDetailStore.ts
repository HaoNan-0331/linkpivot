 ...
 *
 * ⚠ Pattern 3 红线：订阅者仅 DetailsPanel、AppFrameResizer、AIPage 三处——AIPage 为
 * mount-only getState 快照消费（consumePendingAiDevice 一次性读清，不订阅）；
 * MainLayout 与 center 子树对 deviceDetailStore 禁止订阅。TopologyPage 只取 action
 * 引用（写完即走，store 变化零重渲染）。
 *
...
interface DeviceDetailStore {
  ...
  /** 编辑保存成功 bump +1（38-02 面板订阅重拉 getById 的信号线） */
