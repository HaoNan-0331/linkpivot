...
verified: 2026-07-05T08:25:00Z
status: human_needed
score: 4/4 must-haves verified (SC#4 真实句柄计数 deferred to HV)
overrides_applied: 0
re_verification:
...
| 4 | SC#4: 反复触发采集/发现循环后无句柄泄漏（事件句柄/timer/client 计数稳定） | ⚠ STATIC VERIFIED / HV PENDING | 代码级前置达标：三函数（executeSSH/executeTelnet/executeCommandsOnDevice）均 cleanup 统一出口 + settled-flag + try/catch/finally；CR-01 execOne stream.on('error') 已补（ai.ts:414）；CR-02 ready 回调 `if(settled) return` 防 use-after-destroy（ai.ts:363）；WR-03 finally 死分支已改 no-op（ai.ts:387-395）。但**真实句柄计数验证需 Electron 人工 HV**（DEP-1：native binding ABI 不兼容 plain node，mock client 不持真实 socket）→ HV-1~3 pending |

**Score:** 3/4 truths fully VERIFIED + 1/4 static-VERIFIED-but-HV-pending (SC#4)
...
| `electron/services/ai.ts` | executeCommandsOnDevice try/finally + cleanup + clearTimeout + client.end + timeout 路径 client.destroy | ✓ VERIFIED | exists, substantive, wired（discovery.ts:202 调用）；签名零改；isCommandAllowed line 318 保留；overallTimeout 公式 line 330 零改；execOne stream.on('error') 已补（CR-01 fix） |
...
| ai.executeCommandsOnDevice | ssh2 Client 句柄回收 | cleanup: clearTimeout(overallTimer) + client.end，timeout 路径 client.destroy | ✓ WIRED | ai.ts:333-352；CR-02 已修（ready 回调 if(settled) return，commit e6e3381） |
| ai.execOne | ssh2 stream 句柄回收 | stream.on('error') + stream.on('close') | ✓ WIRED | ai.ts:414-415；CR-01 已修（commit f810137） |
...
| --- | --- | --- | --- | --- |
| ROBUST-01 | 06-01 | arpCollector executeSSH/executeTelnet try/finally + client.end/destroy + error 路径 clearTimeout（D-6-1 扩展含 ai.executeCommandsOnDevice） | ✓ SATISFIED | 三函数 cleanup 统一出口；executeTelnet 补自有 setTimeout；CR-01/CR-02 已修；commits eef3004/2389bd8/f810137/e6e3381/37448ea |
| ROBUST-02 | 06-02 | discovery JSON parse 失败带错误上下文 + createSystemLog 调用 try/catch | ✓ SATISFIED | safeLog + enrichParseError helper；5 处全替换；command parse 补 safeLog；commits 16cf9a4/9ad4040/defc698 |
...
| --- | --- | --- |
| D-6-1 范围：三函数全覆盖（executeSSH + executeTelnet + executeCommandsOnDevice）+ executeTelnet 补自有 setTimeout | arpCollector.ts:24-78（executeSSH）+ 80-118（executeTelnet 含 line 87 自有 setTimeout）；ai.ts:308-397（executeCommandsOnDevice） | ✓ |
| D-6-2 模式：cleanup 统一清 timer + client.end；timeout 路径额外 destroy；end/destroy try/catch 幂等 | 三函数 cleanup() 形态一致；arpCollector.ts:48/90/115 destroy 仅 timeout 路径；ai.ts:349 destroy 仅 timeout 路径；try/catch 包裹全覆盖 | ✓ |
...
| D-6-5 静态 grep + Electron 人工 HV（不自动化句柄计数） | 静态 grep 全达标（见 Behavioral Spot-Checks）；HV-1~4 步骤齐全于 06-HUMAN-UAT.md，pending 人工执行 | ✓ 静态部分 / ⚠ HV 部分 pending |

### 红线未回退

- ✓ 三函数签名零改：executeSSH/executeTelnet 仍 `Promise<string>`；executeCommandsOnDevice 仍 `export function (device, commands): Promise<Array<{command,output,success}>>`（ai.ts:308-311）
- ✓ isCommandAllowed 执行层强制校验保留（ai.ts:318；discovery.ts:191-198 discovery 侧也有）
...
- ✓ overallTimeout 公式零改（ai.ts:330 `30000 + commands.length * 15000`）
- ✓ per-command 失败不阻断逻辑保留（ai.ts:372-374 `执行失败: ${err.message}` success:false）
- ✓ commands.length===0 短路保留（ai.ts:312）

### Code Review 修复确认（06-REVIEW.md 2 Critical + 3 Warning）

| Finding | 修复 commit | 核实 |
| --- | --- | --- |
| CR-01 execOne 缺 stream.on('error') | f810137 | ai.ts:414 `stream.on('error', (e: Error) => reject(e))` ✓ |
| CR-02 ready 回调 use-after-destroy race | e6e3381 | ai.ts:363 `if (settled) return` ✓ |
...
| WR-03 finally 有害 cleanup() 分支 | a246f15 | ai.ts:387-395 finally no-op（仅注释，不调 cleanup）✓ |
...
1. **HV-1 SSH ARP 采集反复触发无句柄泄漏（SC#4 / ROBUST-01）**
   - Test: 连 1-2 台真实 SSH 设备，触发 5+ 轮 collectFromAll()，每轮后 process._getActiveHandles() 快照
   - Expected: total 回落基线（±2），Socket/TLSSocket 不单调增长
   - Why human: DEP-1 限制 + 需真实 SSH 设备

2. **HV-2 Telnet 采集 + discovery 反复触发无句柄泄漏（SC#4 / ROBUST-01）**
   - Test: discovery 2-3 轮 + Telnet 采集，句柄快照
   - Expected: handle total 回落，无残留 Telnet socket（验证 executeTelnet 补的自有 setTimeout 与 executeCommandsOnDevice try/finally）
   - Why human: 同 HV-1

3. **HV-3 error / timeout 路径 finally 兜底（SC#1 + SC#4 / ROBUST-01）**
   - Test: 错误凭证 / 192.0.2.1 不可达 IP / 超时设备触发采集，快照前后
   - Expected: handle 回落基线无残留，错误文案透传（如 'SSH timeout after 30000ms'），timeout fire 路径 client.destroy 生效
   - Why human: 需真实网络错误场景

4. **HV-4 discovery JSON parse 失败错误上下文 + createSystemLog 非致命（SC#2 + SC#3 / ROBUST-02）**
   - Test: 4a mock AI 返回非 JSON 验 errorMessage 含 '| 原始片段:'；4b command parse 失败查 ai_system_logs；4c 模拟 DB 写库失败验证主流程不中断 + console.warn 兜底
   - Expected: errorMessage 含原始片段，ai_system_logs 落 failed 记录，DB 失败时主流程正常 + console '[safeLog] discovery 日志写库失败'
   - Why human: 需 Electron 运行时 + DB 写库失败模拟
...
唯一未闭环项是 SC#4 的**真实句柄计数验证**——DEP-1 限制下无法 plain node 自动化（与 Phase 3 HV-1~5 / Phase 5 FE-HV 同模式），已写入 06-HUMAN-UAT.md HV-1~4 步骤齐全 pending 人工执行。代码级前置（cleanup 统一出口 + settled-flag + CR-01 stream error + CR-02 use-after-destroy 防护）已达标，但 status 按 verifier 规则定为 `human_needed`（人工 HV 项非空优先于 passed）。

---

_Verified: 2026-07-05T08:25:00Z_
