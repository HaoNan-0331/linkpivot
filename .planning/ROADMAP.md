...
- [x] **Phase 12: Test Infrastructure (DEP-1 ABI 缓解)** - electron-vite + vitest Electron 内测试通道，补 SSH/Telnet/DB 真路径自动化回归 + 句柄泄漏自动化 (completed 2026-08-08)
- [x] **Phase 13: Security Hardening Cluster** - SSH connection.ts 现代算法 + pre-release 安全项收尾 + experience:list IPC 入参校验防廉价 DoS (completed 2026-08-09)
- [x] **Phase 14: Defect & Legacy Rollback Closure** - BUG-1 new_ip 计数修复 + 旧规划回退项甄别修复（confirm 防重复/ai_exec_logs/会话标题/H3C LLDP） (completed 2026-08-09)
...
- [x] 12-01-PLAN.md — 测试基础设施主干（双 vitest config + test:electron script + 四个 helper + DB 真路径 + SC4 git diff 兜底）
...
- [x] 12-03-PLAN.md — 句柄泄漏专项（异常场景）+ build-smoke.yml CI 扩展（CI-A/B/C 用户决策 + GHA 实跑确认）
...
- [x] 13-02-PLAN.md — SEC-04：pre-release 5 项（L1/L2/L3/L4/L6）逐项甄别登记（DEFER-LOG）+ L6 authGuard 单测扩展确认（核心已满足则显式 defer，不照单全修不静默跳过）
- [x] 13-03-PLAN.md — SEC-05：experience:list IPC 网关层 severity throw + search≤100/tags≤20/单tag≤30 截断防廉价 DoS + VALID_SEVERITIES export 消 drift + mock 单测
...
- [x] 14-02-PLAN.md — FIX-02 旧规划 4 项甄别：#1 confirm 视觉层防重复增强（confirmInFlight 按钮 loading+disabled，核心关窗锁不变）+ #2 ai_exec_logs FIXED 已满足 + #3 会话标题 FIXED 前提偏差 + H3C LLDP 作废，产出 14-02-DEFER-LOG.md（复用 13-02 结构）
...
| 14. Defect & Legacy Rollback Closure | v1.2 | 2/2 | Complete    | 2026-08-09 |
...
| TEST-02 | Phase 12 | Done (12-02 四条 cleanup 路径 + 12-03 异常场景 5 it 句柄泄漏检测全绿，Phase 6 SC#4 + Phase 3 defer 闭合) |
