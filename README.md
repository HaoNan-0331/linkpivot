...
- **拓扑可视化与整理** — React Flow 画布展示网络拓扑；SSH 自动发现设备连接关系（AI 分析 ARP / 路由 / 接口数据推断）；一键星型分层自动布局（全图 / 选区 / 以指定设备为中心）、对齐参考线、拖拽防重叠
- **设备管理** — 设备台账与凭证管理；凭证 AES-256-GCM 字段级加密，主密钥经 Windows DPAPI（safeStorage）绑定机器落盘；资产一键复制（凭证不继承）；设备命名全局唯一（实时查重 + 存量重名扫描引导）
- **远程连接** — SSH / Telnet / Web / RDP 四类远程通道，独立终端窗口（xterm.js），按窗口隔离会话
- **AI agentic 运维助手** — 按问题类型分档自动检索知识库 / 经验 / 实时设备数据再作答；白名单内多步骤连续自主执行（越权 / 白名单外 / 高危才打断确认）；步骤实时可视、可随时中断；回答标注数据来源
- **越权防线** — AI 命令目标越权（命令中出现非目标设备）、借道横向移动（ssh / telnet 跳板）、MCP 工具越界三场景检测，聚合强确认 + 全量审计
- **MCP 包扩展** — 导入 .mcpb 工具包扩展 AI 能力（manifest 校验 / SHA-256 指纹防投毒）；配置绑定设备，环境变量元数据化管理
...
### 网络拓扑
可视化画布展示全网拓扑：添加设备后可通过 SSH 自动发现（AI 分析 ARP / 路由 / 接口数据推断连接关系），也可手动连线；设备节点可直接发起远程连接。**拓扑整理**：一键星型分层放射布局（核心设备居中，按连接层级向外扩散；支持全图 / 选区 / 以指定设备为中心三种模式），布局预览可撤销；拖拽带对齐参考线与防重叠弹开。

### 设备管理
设备台账：录入设备信息（IP / 厂商 / 型号 / 备注）与登录凭证（SSH / Telnet / Web / RDP）。凭证加密存储，界面上只显示脱敏形式。每台设备可一键打开对应类型的远程连接（SSH / Telnet 独立终端窗口，RDP 远程桌面，Web 浏览器）。**资产复制**：一键复制已有设备预填新建表单（凭证需重输，不继承）；**命名唯一**：设备名全局唯一强拦截（含空格 / 大小写 / 连字符变体归一化判定），存量重名自动扫描并引导改名。
...
- **分档检索，该调必调**：按问题类型（故障排查 / 配置查询 / 知识问答 / 巡检）自动检索知识库、经验库、实时设备数据后再作答，回答标注数据来源与是否查询了实时数据
- **连续自主执行**：交代任务后，AI 在白名单内多步骤连续执行（查状态 → 分析 → 下配置 → 验证），每步实时展示步骤卡，可随时中断；步数 / 失败熔断 / 冷却时间 / token 预算四重硬顶防失控
- **命令安全**：所有命令强制过白名单校验（分隔符注入拦截 + 黑名单 + 首词严格匹配），高危命令弹窗二次确认
- **越权防线**：AI 命令中出现非目标设备（目标越权）、借 ssh / telnet 跳转其他设备（横向移动）、调用超出绑定范围的 MCP 工具（越界）时强制打断并聚合确认，全部命中留痕审计
- 对话可一键总结沉淀为知识库经验

### MCP 包
导入 `.mcpb` 工具包为 AI 扩展工具面（如防火墙 API 工具包）：四步导入向导，包完整性五向量校验（zip-slip / 双扩展 / manifest 谎报 / 变量投毒 / 畸形拒绝）+ SHA-256 指纹 spawn 前重验；配置绑定到具体设备，环境变量按包元数据带说明配置、必填项硬校验。
...
### 日志审计
AI 命令执行日志全量审计：完整记录每条命令的输入输出与结果、越权命中与处置终态，可追溯。

### 软件升级
启动时自动检测 GitHub Releases 新版本，有新版弹窗展示升级详情，应用内下载安装；支持「不再提醒」档位（30 天 / 180 天 / 永久）与手动检查入口。下载源不可达时提示并推荐手动下载。

### 设置
系统配置与账号管理；AI 提示词管理（全部内置提示词可查看与覆盖修改，升级时差异比对）；Agent 参数（最大步数 / 失败熔断次数 / 冷却时间）。

## 🛠 从源码构建

环境要求：Windows、Node.js 20.19+、npm
