...
import {
  ...
  getDeviceByIdInternal, guardCheckCommand, toGuardRef, setAiExecMasterKey,
} from './aiExec'
import {
  buildMcpContexts, runMcpCall, setAiMcpMasterKey,
...
// runAgentLoop 族及 pushAgentStep/settleAgentStep 等供本文件剩余 chat/confirm 段消费（纪律 #7
// tsc 证明加 export），mcpResultsUserMessage/runAgentLoopInner 等纯段内函数保持非 export；
// pendingBatches（confirm 批次存储，属 chat 编排域 P4 迁 aiChat）export 供 aiAgentLoop
// confirm 门挂批消费（Shared Pattern 6 运行时晚期环先例）；此处 re-export 保消费方
// import 路径零改动） ----------
